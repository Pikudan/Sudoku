...
class CustomDiffusionTrainer(Trainer):
    ...
    def save_predictions(
        ...
        with open(output_prediction_file, "w", encoding="utf-8") as writer:
            for start in range(0, len(predict_results.label_ids), chunk_size):
                end = min(start + chunk_size, len(predict_results.label_ids))
                preds = np.where(
                    predict_results.predictions[start:end] != IGNORE_INDEX,
                    predict_results.predictions[start:end],
                    self.tokenizer.pad_token_id
                )
                labels = np.where(
                    predict_results.label_ids[start:end] != IGNORE_INDEX,
                    predict_results.label_ids[start:end],
                    self.tokenizer.pad_token_id
                )
                decoded_preds = self.tokenizer.batch_decode(preds, skip_special_tokens=True, clean_up_tokenization_spaces=True)
                decoded_labels = self.tokenizer.batch_decode(labels, skip_special_tokens=True, clean_up_tokenization_spaces=True)
                for pred, label in zip(decoded_preds, decoded_labels):
                    writer.write(json.dumps({"label": label, "predict": pred}, ensure_ascii=False) + "\n")
    ...
    def generate_samples(self, inputs):
        ...
        for t in range(self.diff_args.diffusion_steps-1, -1, -1): # t from T-1 to 0
            with torch.no_grad():
                ...
                if self.diff_args.decoding_strategy.split("-")[0] == "margin":
                    probs = torch.softmax(scores, dim=-1)
                    top2_probs = probs.topk(k=2, dim=-1).values
                    x0_scores = top2_probs[:, :, 0] - top2_probs[:, :, 1]
...
def topk_decoding(
        ...
        elif topk_mode in ["deterministic", "margin"]:
